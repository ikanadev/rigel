	...
	const classPeriodsWithActs = createMemo(() => {
		return classPeriods.map((cp) => ({
			...
			areas: year.areas.map((area) => ({
				...area,
				acts: acts.filter(
					(act) => act.class_period_id === cp.id && act.area_id === area.id,
				),
			})),
		}));
	});
				...
				const areaScores = classPeriodsWithActs().map((cp) => {
					return cp.areas.map((area) => {
						...
						const sum = area.acts.reduce((res, act) => {
							if (st.scoresMap[act.id] !== undefined)
								res += st.scoresMap[act.id].points;
							return res;
						}, 0);
						return Math.round((sum * area.points) / (area.acts.length * 100));
					});
				});
				const periodScores = areaScores.map((ag) =>
					ag.reduce((res, a) => res + a, 0),
				);
				const yearScore = Math.round(
					periodScores.reduce((res, pg) => res + pg, 0) / periodScores.length,
				);
				return {
					...st,
					areaScores,
					...
					<Flex alignItems="center">
						...
						<Box mr="$4" minW="$48">
							<SimpleSelect value={viewMode()} onChange={setViewMode} size="sm">
								<For each={Object.values(ViewMode)}>
									{(level) => (
