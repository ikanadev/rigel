	...
	return (
		<>
			...
			<SimpleGrid
				...
				columnGap="$6"
			>
				<For each={classes}>{(cl) => <ClassItem item={cl} />}</For>
			</SimpleGrid>
			...
			<Show when={classes.length >= maxClasses && !premium}>
				...
				<Text mt="$1" color="$neutral10">
					Con {APP_NAME} premium puedes crear hasta {MAX_CLASSES_PREMIUM}{" "}
					materias, encuentra las instrucciones en el{" "}
					<Anchor
						href={TELEGRAM_LINK}
			...
			<Show when={classes.length >= maxClasses && premium}>
				...
				<Text mt="$1" color="$neutral10">
					{APP_NAME} sólo permite hasta un máximo de {MAX_CLASSES_PREMIUM}{" "}
					materias en la versión premium.
				</Text>
			</Show>
		</>
	);
