...
const CopyStudentsModal: Component<ModalProps> = (props) => {
	...
	const students = createDexieArrayQuery(() =>
		db.students.where({ class_id: classId() }).toArray(),
	);
	const otherClasses = () =>
		classes.filter((c) => c.id !== classStore.class?.id);
			...
			<ModalContent>
				...
				<ModalBody>
					...
					<SimpleSelect
						...
						mb="$2"
					>
						<For each={otherClasses()}>
							{(cl) => (
								<SimpleOption
									value={cl.id}
									textValue={`${cl.subject.name}, ${cl.grade.name} - ${cl.parallel}`}
								>
									<Flex flexDirection="column" fontSize="$sm" lineHeight={1.2}>
										<Text>{cl.subject.name}</Text>
										<Text>
											{cl.grade.name} - {cl.parallel}
										</Text>
									</Flex>
								</SimpleOption>
							)}
					...
					<Show when={students.length === 0 && classId().length > 0}>
						<Alert
							status="warning"
							text="No hay estudiantes en esta materia."
						/>
					</Show>
				</ModalBody>
				<ModalFooter>
