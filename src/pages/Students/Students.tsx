...
const Students: Component = () => {
	const [selectedStudent, setSelectedStudent] = createSignal<Student | null>(
		null,
	);
	...
	return (
		<>
			<CopyStudentsModal
				opened={copyModal.isActive()}
				onClose={copyModal.disable}
			/>
			<DeleteModal
				opened={deleteModal.isActive()}
				onClose={deleteModal.disable}
								...
								{(student) => (
									<Tr>
										...
										<Td numeric>
											<IconButton
												as={Link}
												href={`/class/${classStore.class!.id}/student/${
													student.id
												}/edit`}
												size="xs"
												colorScheme="info"
