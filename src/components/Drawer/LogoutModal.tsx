...
const LogoutModal: Component<Props> = (props) => {
  ...
  const { actions: { clearAll } } = useAppData();
    ...
    <Modal opened={props.isOpen} onClose={props.onClose} size="md">
      ...
      <ModalContent>
        ...
        <ModalBody>
          ...
          <Text my="$2">
            ¿Deseas cerrar sesión?. {missingTxs() > 0 ? ` Se perderán ${missingTxs()} operaciones.` : ''}
          </Text>
        </ModalBody>
        <ModalFooter>
          {/* eslint-disable-next-line solid/reactivity */}
          <Button size="sm" colorScheme="neutral" onClick={props.onClose}>Permanecer en la WebApp</Button>
          <Button ml="$2" size="sm" colorScheme="danger" onClick={handleLogout}>Si, cerrar sesión</Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
  );
