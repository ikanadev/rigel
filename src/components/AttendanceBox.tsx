...
const AttendanceBox: Component<{ status?: AttendanceStatus; active: boolean }> =
	(props) => {
		...
		return (
			...
			>
				<Box
					...
					color={
						coloredAtts() ? attendanceColors[props.status!].on : "$neutral12"
					}
					borderWidth={1}
					borderColor={
						coloredAtts() ? attendanceColors[props.status!].on : "$neutral12"
					...
					bgColor={
						coloredAtts() ? attendanceColors[props.status!].off : "$neutral2"
					}
					fontSize="$xs"
					as="pre"
					px="$0_5"
